...
!     --------------------------------------------------------
      ...
      real :: ydisp = 0.1               !-----ydisp = y value (m) at which contours of D(x, kx) are drawn
      real :: xkydisp = 50.0            !-----xkydisp = ky value (m-1) for which contours of D(x, kx) are drawn
      ...
      logical :: use_new_z2 = .true.      !-----if (use_new_z2 .eq. .true.)  use the toroidally broadened Z2 for electrons with l = 0 (default)
                                          !-----if (use_new_z2 .eq. .false.) use the original Z2 function consistent with nzfun					   					  
     ...
     &    xkx_ono, xky_ono, curved_ant, yplot, use_new_Z2
