...
c
     ...
     .   omgpe2, omgpi2, xme, upshift, nphi, rt)
     ...
     .   nkx1, nkx2, nky1, nky2, myid, nproc, id, ngrid, icontxt
      ...
      integer upshift, nphi

      real x(nxdim), y(nydim), eps0, xmu0
      real xkx(nkdim1 : nkdim2), xky(mkdim1 : mkdim2)
      real z, xmi, omgrf, clight, va2, vthe, pi, deltax
      real sgn_kprl, vthi, xdebye2, rt
      
      complex S, P, R, L, D, aq, bq, cq, Phot, Pcold, descrim
     ...
     .     xkte(nxdim, nydim), xkti(nxdim, nydim)
      ...
      real    xkperp2_slow, xkperp2_fast, xkperp2_disp   
...
c		     cq =   P * (S - xnprl**2)**2 - D**2 * P  
     ...
     .                  - xnprl**2* (1. + delta)**2 * (S - xnprl**2)
		     ...
		     xnperp2_slow = (-bq + sqrt(descrim) ) / (2. * aq)     
		     xnperp2_fast = (-bq - sqrt(descrim) ) / (2. * aq)     
     ...
     .                  diff = abs(xkperp2 - xkperp2_slow)/ abs(xkperp2)	
		     ...
		     if (root .eq. 'fast') 
     .                  diff = abs(xkperp2 - xkperp2_fast) /abs(xkperp2)
