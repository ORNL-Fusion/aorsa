      ...
      do i = 1, nnodex
         do j = 1, nnodey
            do k = 1, nrho-1
               ...
               if(rho_ij(i,j) .le. rho(1)) then !  need one sided to the left
...
c                 call flush(6)
     ...
     &            (rho(2) - rho(1)) * (rho_ij(i,j) - rho(1))
               ...
               elseif(rho_ij(i,j) .ge. rho(nrho)) then ! need one sided to the right.
...
c     &            (rho(nrho) - rho(nrho-1)) * (rho_ij(i,j) - rho(nrho))
     ...
     &            (rho(k+1) - rho(k)) * (rho_ij(i,j) - rho(k))
            ...
            enddo
            if (profile_out(i,j) .le. 0.0) profile_out(i,j) = 1.0e-10
         end do
      end do
