...
!c
       ...
       !c new method for reading in 4d distribution function from p2f, or other code with the listed variable names.
       !c not related to cql3d, but thought would 
       !c be good to keep non-maxwellian stuff together here. JVDL 2025

       ! subroutine netcdfr4d_single_gen(netcdfnm, &
       !                             R_binCenters, z_binCenters, &
       !                             vPer_binCenters, vPar_binCenters, &
       !                             f_rzvv)
       ...
       ! ! ─── Inputs ───
       ! character(len=*) :: netcdfnm

       ! ! ─── Outputs ───
       ! real, allocatable :: R_binCenters(:)
       ! real, allocatable :: z_binCenters(:)
       ! real, allocatable :: vPer_binCenters(:)
       ! real, allocatable :: vPar_binCenters(:)
       ! real, allocatable :: f_rzvv(:,:,:,:)  ! f(R, Z, vPer, vPar)
       ...
       ! istatus = nf90_open(trim(netcdfnm), NF90_NOWRITE, ncid)
       ! if (istatus /= nf90_noerr) stop 'ERROR: Failed to open file.'
       ...
       ! ! ───── Allocate outputs ─────
       ! allocate(vPar_binCenters(n_vpar))
       ! allocate(vPer_binCenters(n_vper))
       ! allocate(z_binCenters(n_z))
       ! allocate(R_binCenters(n_r))
       ! allocate(f_rzvv(n_r, n_z, n_vper, n_vpar))  ! Fortran order: (R, Z, vPer, vPar)
       ...
       ! if (istatus /= nf90_noerr) stop 'ERROR: Could not get varid for f_rzvv'

       ! istatus = nf90_get_var(ncid, vid, f_rzvv)
       ! if (istatus /= nf90_noerr) stop 'ERROR: Could not read f_rzvv'
       ...
       ! ! ───── Get the dimension length of a 1D variable ─────
       ! subroutine get_dimlen(ncid, varname, len_out)
       ! integer :: ncid
       ! character(len=*) :: varname
       ! integer :: len_out
       ! integer :: vid, dimid, istatus
       ...
       ! istatus = nf90_inquire_variable(ncid, vid, dimids = (/dimid/))
       ! if (istatus /= nf90_noerr) stop 'ERROR: Could not get dim ID.'

       ! istatus = nf90_inquire_dimension(ncid, dimid, len = len_out)
       ! if (istatus /= nf90_noerr) stop 'ERROR: Could not get dimension length.'
       ! end subroutine get_dimlen

       ! ! ───── Read 1D variable (like bin centers) ─────
       ...
       ! istatus = nf90_inq_varid(ncid, varname, vid)
       ! if (istatus /= nf90_noerr) stop 'ERROR: Could not get varid for '//trim(varname)

       ! istatus = nf90_get_var(ncid, vid, array)
       ! if (istatus /= nf90_noerr) stop 'ERROR: Failed to read '//trim(varname)
