...
c
      ...
      subroutine field_line_trace(myid, eqdsk, nmodesx, nmodesy, 
     ...
     &   profile_out(:,:)
      ...
      real s_err, y_len, sgn_vprl, modb, bratio_phi, modb_init
...
c      double precision ws(lenws)
     ...
     &     dthetant, gaussantth, xnuomg, gaussiant, lenmax
...
c            capr_x0 = capr(i) + dx / 2.0
c            capz_x0 = y(j) + dy / 2.0      
            capr_x0 = capr(i) 
            capz_x0 = y(j)          
...
*           -------------------------------------------
c           lenmax = twopi * capr(i) * qsafety(i,j)
            lenmax = twopi * capr(i)
...
*              --------------------------------------------
               ...
               if(myid .eq. 0) then
                  write(6, 2213) n_len, h0, 
     &            capr_x(n_len), capz_x(n_len), phin_x(n_len),
     &            bratio_phi, fcount         

                  write(115, 2213) n_len, h0, 
     &            capr_x(n_len), capz_x(n_len), phin_x(n_len),
     &            bratio_phi, fcount   
               end if
     
 2213          format(1i10, 1p5e12.4, 3i5)     

               h0 = twopi / 360.                
               if(len .gt. lenmax)go to 201

                
            end do       ! end of n_len loop along orbit !
*           -----------------------
...
*           -----------------------
            ...
            do nphii = nphi_enter + 1, nlen_exit - 1
                     dtau_sum = dtau_sum + dl_vprl(nphii)
            end do
                  
            dtau_sum = dtau_sum + 0.5 * dl_vprl(nphi_enter)
            dtau_sum = dtau_sum + 0.5 * dl_vprl(nlen_exit)
...
9318  format(a128)
 ...
 1214 format(1p4e12.4, 3i5)
      ...
      write(138, 309) n_len_max
      write(138, 310) (capr_x(n_len), n_len = 1, n_len_max)
      write(138, 310) (capz_x(n_len), n_len = 1, n_len_max)
...
13149 format (4i10, 1p,8e12.4)
 ...
 1009 format(3x,21h        frequency  = ,1p,e12.4,7h hertz )
 1012 format(3x,21h             omgrf = ,1p,e12.4,7h hertz )
 2012 format(3x,51h2*pi*rt/Ly * real part of impedance (resistance) = ,
     1   1p,e12.4,10h ohms     )
 2013 format(3x,
     1   55h2*pi*rt/Ly * imaginary part of impedance (reactance) = ,
     1   1p,e12.4,10h ohms     )
 1014 format(3x,21h               xkz = ,1p,e12.4,7h m-1   )
 1321 format(3x,21h         vph / vth = ,1p,e12.4,7h       )
 1391 format(3x,21h critical shear(0) = ,1p,e12.4,7h s-1   )
 1392 format(3x,21h critical shear(a) = ,1p,e12.4,7h s-1   )
 1393 format(3x,21h         mu neo(a) = ,1p,e12.4,7h s-1   )
 1322 format(3x,21h               vph = ,1p,e12.4,7h       )
 1323 format(3x,21h               vth = ,1p,e12.4,7h       )
 1714 format(3x,21h               xk0 = ,1p,e12.4,7h m-1   )
 1021 format(3x,21h        n parallel = ,1p,e12.4,7h       )
 1022 format(3x,21h           rhonorm = ,1p,e12.4,7h m     )
 1812 format(3x,21h                rt = ,1p,e12.4,7h m     )
 1822 format(3x,21h            aplasm = ,1p,e12.4,7h m     )
 1823 format(3x,21h              rant = ,1p,e12.4,7h m     )
 1809 format(3x,21h                b0 = ,1p,e12.4,7h T     )
 1813 format(3x,21h              xn10 = ,1p,e12.4,7h m-3   )
 6813 format(3x,21h              xne0 = ,1p,e12.4,7h m-3   )
 1814 format(3x,21h              xn20 = ,1p,e12.4,7h m-3   )
 1834 format(3x,21h              xn30 = ,1p,e12.4,7h m-3   )
 6834 format(3x,21h              eta1 = ,1p,e12.4,7h       )
 6835 format(3x,21h              eta2 = ,1p,e12.4,7h       )
 6836 format(3x,21h              eta3 = ,1p,e12.4,7h       )
 1815 format(3x,21h               te0 = ,1p,e12.4,7h eV    )
 1821 format(3x,21h               ti0 = ,1p,e12.4,7h eV    )
 1016 format(3x,21h xnue/omgrf ad hoc = ,1p,e12.4,7h       )
 1017 format(3x,21h xnu1/omgrf ad hoc = ,1p,e12.4,7h       )
 1018 format(3x,21h xnu2/omgrf ad hoc = ,1p,e12.4,7h       )
 ...
 7016 format(3x,21h         rhoi1 / L = ,1p,e12.4,7h       )
 7017 format(3x,21h             rhoi1 = ,1p,e12.4,7h m     )
 7217 format(3x,21h             qavg0 = ,1p,e12.4,7h       )
...
30132 format(3x,21h            iright = ,i12,7h       )
 3014 format(3x,21h xnuii(0)/omgti(0) = ,1p,e12.4,7h s-1   )
 3015 format(3x,21h           vthi(0) = ,1p,e12.4,7h m/s   )
 3016 format(3x,21h          omgti(0) = ,1p,e12.4,7h s-1   )
 3017 format(3x,21h           xnup(0) = ,1p,e12.4,7h s-1   )
 3018 format(3x,21h          eps**1.5 = ,1p,e12.4,7h       )
71160 format(3x,21h         xnu / omg = ,1p,e12.4,7h       )
