...
!*************************************************************************
!
    subroutine QLSUM_MAXWELLIAN_z2_ion(ip, jp, k_uper, b_sum, c_sum,  & 
       & e_sum, f_sum, &
       & sum_wdot, sum_fx0, sum_fy0, W, ZSPEC,  &
       & ASPEC, BMAG, lmax, ENORM, UPARMIN, UPARMAX, &
       ...
       & lmaxdim, ndist, nzeta, &
       & gradprlb, bmod, omgc, alpha, xm,   &
       & upshift, xk_cutoff, rt, nphi, rho,  &
...
!            ---------------------------------            
     ...
     &               + xkphi     * bhatgradbz	     
	     ...
	     if (rho .gt. 1.0) then
             ...
             ldum = 1
	     if(xkprln .eq. 0)xkprln = 1.0e-06
             gammab = abs(ldum * omgc / (2.0 * alpha * xkprln**2)  &
     &                                    * gradprlb / bmod)
             if(xm .eq. xme)gammab = 0.0
             if(abs(gammab) .gt. 1000.0) gammab = 1000.0
             if(abs(gammab) .lt. .01)gammab = .01
...
!    end if
  ...
  end subroutine QLSUM_MAXWELLIAN_z2_ion
