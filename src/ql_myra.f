...
c
      ...
      subroutine wdot_4df(bqlavg, cqlavg, eqlavg, fqlavg, 
     &   wdot_inout, fx0_inout, fy0_inout, fz0_inout, 
     &   vol, nrhodim, nnoderho, drho,
     &   dx, dy, r0, nxdim, nydim, nnodex, nnodey,
     &   nkx1, nkx2, nky1, nky2, nkdim1, nkdim2, mkdim1, mkdim2,
     ...
     &   nupar, nuper, n_psi,
     &   n_psi_dim, dfduper, dfdupar, fperp, 
     &   UminPara_4df, UmaxPara_4df, UPERP, UPARA,
     &   vc_mks_4df, df_cql_uprp, df_cql_uprl, rho, rho_a, nbessj,
     &   zeffcd, clight, x, y, rt, b0, ftrap, omgrf,
     &   nkperp, lmaxdim, nzeta_wdot, theta_,
     &   n_theta_max, n_psi_max, i_psi_eq, n_theta_, dldbavg, 
     &   n_bin, upshift, i_write, xk_cutoff, n_r, n_z, 
     &   n_vper, n_vpar, f_rzvv_local, 
     &   dfdvper_local, dfdvpar_local, vPer_rzvv, vPar_rzvv)

*----------------------------------------------------------------------
*     JVDL - This subroutine calculates wdot for the 4df in nearly the 
*     same way as ql_myra_write, it just skips mapping off
*     midplane, and instead reads dfdu directly. The QL operator is skipped for now.
*     In the future, just look to ql_myra_write, and skip bounce
*     averaging, and write the QL operator directly to a 4d R,Z,vperp,vpar
*     grid. 
*----------------------------------------------------------------------
...
c      real, dimension(:,:), allocatable :: factvol2d
      ...
      real :: W, ENORM, ZSPEC, ASPEC, BMAG
      real :: UminPara,UmaxPara
      real :: UminPara_4df,UmaxPara_4df
      real :: df_cql_uprp(NUPER, NUPAR, n_psi_dim)
      real :: df_cql_uprl(NUPER, NUPAR, n_psi_dim)
      real :: vc_mks, vc_mks_4df, rho_a(n_psi_dim)
      real :: eps0, pi, emax, u0, u_0, u_, costh, costh0, psic
...
c      factvol2d = 0.0      
      ...
      vc_mks = vc_mks_4df
      UminPara = UminPara_4df
      UmaxPara = UmaxPara_4df
...
!           Assumes uniformly spaced u grids            
            duperp = (uperp(nuper) - uperp(1)) / (nuper - 1)
            dupara = (upara(nupar) - upara(1)) / (nupar - 1)
...
!           -----------------------------------------------------
!           get 4df distribution function on the midplane:
!           used for Wdot only - not the quasilinear coefficients
!           -----------------------------------------------------
