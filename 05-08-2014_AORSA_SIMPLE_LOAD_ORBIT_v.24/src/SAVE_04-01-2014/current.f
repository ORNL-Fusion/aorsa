...
c                        c_ehst = 1.0
     ...
     .                     + uzz(i,j) * xkphi(i)
                     ...
                     xkprl_phi = uzz(i, j) * xkphi(i)     
                     xkprl_perp = uzx(i, j) * xkxsav(n) 
     .  		        + uzy(i, j) * xkysav(m)        
...
*                          ---------------------------------------
*                          fit to circular helix with secular term                      
*                          ---------------------------------------
...
*                       --------------------------------------------------
*                       Calulate sigma_e (l = 0) for each k in the Fourier 
*                          expansion along the field line - xklsav(nkl)
*                       -------------------------------------------------- 
                        do nkl = nkl1, nkl2
     ...
     .                        mtable, sige3_new)    
     			   ...
     			   ekl = cexp(zi * xklsav(nkl) * lbprime(nl))   
                           ...
                           Jxxsum = Jxxsum + Eln(nkl) * sigxx * ekl 
                           Jxysum = Jxysum + Eln(nkl) * sigxy * ekl
                           Jxzsum = Jxzsum + Eln(nkl) * sigxz * ekl 
     
                           Jyxsum = Jyxsum + Eln(nkl) * sigyx * ekl 
                           Jyysum = Jyysum + Eln(nkl) * sigyy * ekl 
                           Jyzsum = Jyzsum + Eln(nkl) * sigyz * ekl 
     
                           Jzxsum = Jzxsum + Eln(nkl) * sigzx * ekl 
                           Jzysum = Jzysum + Eln(nkl) * sigzy * ekl 
                           Jzzsum = Jzzsum + Eln(nkl) * sigzz * ekl			  			   			      
                                 			      
                        end do
			   
		        sigexx_new = Jxxsum / Esum
                        sigexy_new = Jxysum / Esum
                        sigexz_new = Jxzsum / Esum
			   
                        sigeyx_new = Jyxsum / Esum
                        sigeyy_new = Jyysum / Esum
                        sigeyz_new = Jyzsum / Esum
			   
                        sigezx_new = Jzxsum / Esum
                        sigezy_new = Jzysum / Esum
                        sigezz_new = Jzzsum / Esum
			
                        sige3_new = sigezz_new
